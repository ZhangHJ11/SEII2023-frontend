...
import {toByte, toByteArray} from "~/utils/ticket"
...
const addTrain = (train: TrainInfo) => {
  ...
  let ticket_infos = [] as Array<string>
  train.ticket_infos.forEach(function (item) {
    ticket_infos.push(JSON.stringify(item).toString());
  })
  // console.log(ticket_infos)
  // console.log(toByte(ticket_infos))
  ...
  request({
    ...
    data: {
      ...
      // ticket_infos: toByte(ticket_infos)
      // test: toByte(train.ticket_infos[0])
    }
  }).then((res) => {
    ElNotification({
      offset: 70,
